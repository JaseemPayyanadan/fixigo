...
function RegisterContent() {
  ...
  const assignRole = async (uid: string, name: string, email: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await assignRole(userCredential.user.uid, name, email);
      // Redirect to shop onboarding after successful registration
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Assign role only if user is new (first sign-in)
      if (additionalInfo?.isNewUser) {
        await assignRole(user.uid, user.displayName || "", user.email || "");
        // Redirect to shop onboarding for new users
        router.push("/shop-onboarding");
      } else {
        // If not new, check if user doc exists, if not, assign role
        ...
        if (!userDoc.exists()) {
          ...
          if (userData.role === "shop_admin" && !userData.shopId) {
            // Shop admin without shopId needs onboarding
            router.push("/shop-onboarding");
          } else {
