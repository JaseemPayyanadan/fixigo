...
export function AppBar() {
  ...
  return (
    <header className={`fixed left-0 right-0 top-0 z-[9999] flex h-14 items-center gap-3 border-b border-gray-100 bg-white px-3 transition-all duration-200 md:gap-4 md:px-6 ${rail ? "md:ml-14" : "md:ml-56"}`}>
      {/* Sidebar toggle */}
      <button
        onClick={() => setCollapsed(!collapsed)}
        aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
        className="hidden h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 lg:flex"
      >
      ...
      <div className="flex flex-1 items-center md:hidden">
        <FixigoLogo variant="lockup" size={24} />
      </div>
      ...
      {/* Date */}
      <div className="hidden shrink-0 items-center gap-2 rounded-lg border border-gray-200 px-3 py-1.5 text-sm font-semibold text-gray-700 xl:flex">
